...
def mock_moose_process(self, *_, **__):
    ...
    temp_logfile = tempfile.NamedTemporaryFile(
        mode="w", prefix="moose_test_", suffix=".csv", buffering=1
    )
    ...
    def write_to_csv(temp_logfile=temp_logfile):
        log_file = (
            ...
            .parent.joinpath("example_data", "moose_temps_avgs.csv")
            .open("r")
        )
...
def test_scalar_pp_parser(folder_setup):
    ...
    sample_metric = client.get_metric_values(
        metric_names=["handle_temp_avg"],
        xaxis="time",
        output_format="dataframe",
        run_ids=[run_id],
    )
    assert list(sample_metric.index.levels[0]) == [
        0.0,
        2.0,
        4.0,
        6.0,
